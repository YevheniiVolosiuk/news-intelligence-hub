...
const DEMO_ARTICLES: ArticleDefinition[] = [
  ...
  // ── Reuters (source 1, feed 1) ─────────────────────────────────
  {
    ...
    normalisedUrl: 'https://www.reuters.com/world/europe/nato-summit-defence-spending',
  ...
  // ── BBC (source 2, feed 2) ─────────────────────────────────────
  {
    ...
    normalisedUrl: 'https://www.bbc.co.uk/news/technology-quantum-computing-breakthrough',
  ...
  },
  {
    ...
    title: '',
    content: '<div class="cookie-banner">We use cookies to improve your experience</div>',
...
export async function seedDemoArticles(
  ...
  for (const def of DEMO_ARTICLES) {
    ...
    if (!sourceId) {
      throw new Error(`Demo seed: no source found for host "${def.sourceHost}"`);
    }

    const feedId = feeds[def.feedIndex]?.id;
    if (!feedId) {
      throw new Error(`Demo seed: no feed found at index ${def.feedIndex} for "${def.url}"`);
    }
    ...
    if (rows.length > 0) {
      articles.push({id: rows[0].id, processingState: rows[0].processing_state});
    } else {
